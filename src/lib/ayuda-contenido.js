...
const AYUDA = {
  ...
  'empresas': { titulo: 'Empresas', que: 'Las entidades legales del grupo (ejecutoras y proveedoras internas).', como: ['El selector de arriba elige QUÉ mirás: las empresas DEL GRUPO (lo normal), los CONSORCIOS o los TERCEROS. Un consorcio no se edita acá sino desde su obra; un tercero es un proveedor o cliente y, aunque no salga en esta lista, sigue disponible en todos los selectores contables.', 'REVISAR CLASIFICACIÓN (admin) separa las tres cosas de una vez. Ojo: hay proveedores que se LLAMAN "Consorcio" y no lo son — la pantalla propone según si la empresa ejecuta alguna obra nuestra, no según el nombre, y muestra la evidencia para que la corrijas. Reclasificar no mueve ningún movimiento contable.', 'Las empresas que Captura Mágica crea sola al leer una factura nacen como TERCEROS: por eso ya no ensucian este catálogo.', 'El RUC y la clase de cada empresa alimentan el cruce INTERCO y los filtros de emisor/receptor.', 'El RUBRO y el checkbox "📸 Mostrar en el portal de campo" controlan la tabla de RUCs que ve el personal de obra al subir facturas: solo salen las empresas marcadas.', 'Un CONSORCIO no tiene "Ver detalle": su fila lleva "Ir a su obra", porque su contabilidad, personal y trabajo viven en el panel de la obra que ejecuta (entrega B), no acá.', 'VER DETALLE (empresas propias) abre la ficha con 4 pestañas. CONTABILIDAD: sus números con el MISMO criterio del Consolidado (una moneda por vez, sin anulados y con lo facturado entre empresas del grupo separado). INVENTARIO: QUÉ COMPRÓ — cada insumo con sus cantidades (unificando las formas en que el OCR escribe la unidad: "und", "unidad", "each"), el gasto por moneda, los proveedores, la última compra y las facturas detrás; si además revende, cuánto de ese insumo volvió a salir vendido y el saldo. PERSONAL: se DERIVA, no es una columna nueva — es la gente de las obras que esta empresa ejecuta o de las que es socia, agrupada por forma de pago (planilla / recibo por honorarios / sin definir). TRABAJOS: las obras (como ejecutora o socia, con su % si es socia) y los bienes/servicios que ejecuta, con acceso directo al panel de cada uno.', 'OJO con el inventario: es lo COMPRADO según las facturas con detalle de ítems (las de Captura Mágica), NO el stock — los consumos y salidas se llevan en Almacén por obra. Las facturas registradas a mano sin ítems se cuentan aparte y se avisa cuántas son.'] },
  'guias-remision': {
