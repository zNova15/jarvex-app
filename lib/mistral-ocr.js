...
// ── EL PAQUETE SCTR, PÁGINA POR PÁGINA ────────────────────────────
// Hasta el 8-set-2026 el modo `sctr_paquete` iba SIEMPRE por visión directa de
// Claude sobre el PDF, con este motivo escrito en el endpoint: «necesita saber
...
// siempre las tuvo. Rotulando cada una, la fidelidad de página se conserva y el
// paquete se puede estructurar desde texto — que es lo que permite sacarlo de
// Claude y mandarlo al mismo OpenRouter que usa Captura Mágica, para que el
// botón deje de morirse cada vez que se agota el saldo de Anthropic.
//
// El rótulo lleva el TOTAL además del número («PÁGINA 3 de 9») porque la
// respuesta tiene que cubrir 1..N sin huecos: sin el total, el modelo no sabe
// dónde termina y devuelve rangos que `validarSecciones` tiene que recortar.
//
// Una página sin texto legible (un sello escaneado, una firma) NO se omite: se
// deja con su rótulo y una marca. Saltearla correría la numeración de todas las
// siguientes, y esos números son los que después cortan el PDF de verdad.
export function textoPaginadoSctr(paginas) {
  const arr = Array.isArray(paginas) ? paginas : [];
  const n = arr.length;
  return arr
    .map((t, i) => `===== PÁGINA ${i + 1} de ${n} =====\n${String(t || '').trim() || '(página sin texto legible)'}`)
    .join('\n\n');
}
