...
//
// ── REMEDIDO EL 15-set-2026, Y EL MERCADO CAMBIÓ A FAVOR ──────────
// Cuando se escribió lo de arriba, bajo política `zdr` quedaba UN SOLO
// gratuito vivo y los respaldos se salteaban solos. Hoy son DOCE los que pasan
// el filtro `zdr=true`, y dos de ellos aceptan JSON FORZADO — que es justo la
// debilidad anotada de esta cadena («no admite JSON forzado, razona en voz alta
// y a veces se corta»). Por eso el respaldo pasa a ser:
//
//   1. nvidia/nemotron-3-super-120b-a12b:free — 120B MoE, 262k, ZDR Y JSON
//      forzado. Es el gratuito más capaz del catálogo hoy.
//   2. google/gemma-4-31b-it:free — chico pero con ZDR y JSON forzado.
//   3. openrouter/free — el enrutador, que sobrevive a las rotaciones.
//
// El TITULAR no se toca: `ling-3.0-flash-fin:free` es el que viene atendiendo
// Captura Mágica y ahí «funciona bien hoy» (ver lib/modelos-ia.js). Cambiar el
// titular sin medir sería cambiar algo que anda por un número que nadie corrió;
// lo que se amplía es la RED DE ABAJO, que hasta hoy estaba vacía bajo ZDR.
//
// 🔴 Y LO QUE NO EXISTE, PARA QUE NADIE LO BUSQUE: un gratuito con muchos
// proveedores. Los 23 `:free` tienen UN endpoint cada uno; `gpt-oss-120b` tiene
// 24. Esa multiplicidad es exactamente lo que se paga. Una cadena de gratuitos
// es lo más parecido que hay, y es esto.
export const MODELO_FALLBACK_DEFAULT = 'nvidia/nemotron-3-super-120b-a12b:free,google/gemma-4-31b-it:free,openrouter/free';
