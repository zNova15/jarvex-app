...
// 🔴 NINGÚN MODELO GRATUITO ES PERMANENTE. La lista rota sin aviso: los `:free`
// de DeepSeek fueron los más usados de 2025 y a mediados de 2026 estaban todos
// en pago; los tiers gratis de Llama y Qwen desaparecieron antes de agosto.
// Antigüedad medida el 5-sep-2026 (fecha de listado en OpenRouter):
//   openrouter/free ........ 216 días   ← el enrutador, el único diseñado para sobrevivir
//   minimax-m3:free ......... 97 días
//   ling-3.0-flash-fin:free .. 9 días   ← el mejor medido, pero es un recién llegado
// Por eso la cadena va de "el mejor" a "el que va a seguir estando".
...
//
// ── REMEDIDO CONTRA LA API, NO CONTRA EL CATÁLOGO (15-set-2026, noche) ──
// Unas horas antes esta cadena pasó a nemotron-3-super-120b + gemma-4-31b
// porque el catálogo (`/api/v1/models`) los marcaba con `zdr: true`. **El flag
// del catálogo dice qué declara el modelo, NO qué endpoints puede usar ESTA
// cuenta.** Pedidos de verdad, con la key de producción y `provider.zdr:true`,
// los dos contestan:
//   404 "No endpoints found matching your data policy (Zero data retention)"
// Se barrieron LOS 20 gratuitos del catálogo uno por uno. Pasan ZDR TRES, y los
// tres son la misma familia servida por Novita:
//   inclusionai/ling-3.0-flash-fin:free    ← titular (el medido, finanzas)
//   inclusionai/ling-3.0-flash-vl:free     ← respaldo
//   inclusionai/ling-3.0-flash-sante:free
// Los otros 17 dan 404 de política (15) o 403/429 (2). O sea: la promesa de
// "doce gratuitos con ZDR" era del catálogo, no de la realidad.
//
// MORALEJA PARA LA PRÓXIMA ROTACIÓN: un modelo entra a esta cadena después de
// que `scripts/revisar-modelos-openrouter.mjs` le saque un 200 REAL con la
// política puesta. Leer el flag y confiar es cómo se llegó al apagón de hoy.
//
// `openrouter/free` cierra la cadena: es el auto-router, elige solo entre los
// gratuitos vivos y por eso sobrevive a las rotaciones (hoy resuelve, él
// también, a ling-3.0-flash-vl).
//
// 🔴 Y LO QUE NO EXISTE, PARA QUE NADIE LO BUSQUE: un gratuito con muchos
// proveedores. Los `:free` tienen UN endpoint cada uno; `gpt-oss-120b` tiene
// 24. Esa multiplicidad es exactamente lo que se paga. Una cadena de gratuitos
// es lo más parecido que hay, y es esto.
//
// ⚠️ TRES ES EL TOPE, NO UNA PREFERENCIA. Ver MAX_MODELOS_CADENA.
// La cadena gratuita base tiene EXACTAMENTE DOS modelos:
// 1) ling-3.0-flash-fin:free (titular)
// 2) ling-3.0-flash-vl:free (respaldo)
// El tercer cupo queda libre para cuando se elija un modelo adicional (como
// gpt-oss-120b) en las otras herramientas (categorización, correlación, mapeo)
// o en captura, sumando 1 + 2 = exactamente 3 modelos en total (nunca 4).
export const MODELO_FALLBACK_DEFAULT = 'inclusionai/ling-3.0-flash-vl:free';

// OpenRouter rechaza con 400 cualquier `models` de más de 3 entradas — titular
// incluido. No es negociable ni se puede subir con un parámetro: es el límite
// de la API. `construirCuerpo` recorta acá para que una lista larga degrade en
// vez de tumbar la llamada entera.
export const MAX_MODELOS_CADENA = 3;

/**
 * Arma la lista de modelos para OpenRouter respetando el tope de 3:
 * - En 'auto' (cadena gratuita): los 2 modelos gratuitos (titular + 1 respaldo).
 * - Con modelo elegido: el elegido como titular + los 2 gratuitos como respaldo (total = 3).
 * Garantiza que NUNCA supere MAX_MODELOS_CADENA (3).
 */
export function armarCadenaOpenRouter(elegido, cfg = {}) {
  const gratuitoTitular = cfg.modelo || MODELO_DEFAULT;
  const gratuitosRespaldos = (cfg.respaldos && cfg.respaldos.length > 0)
    ? cfg.respaldos
    : [MODELO_FALLBACK_DEFAULT];
  const gratuitos = [gratuitoTitular, ...gratuitosRespaldos].filter(Boolean);

  const esAuto = !elegido || elegido === 'auto';
  if (esAuto) {
    const modelo = gratuitos[0] || MODELO_DEFAULT;
    const respaldos = gratuitos.slice(1, 2);
    return { modelo, respaldos };
  }

  // Modelo elegido como titular + hasta 2 gratuitos como respaldo (total 3)
  const modelo = elegido;
  const respaldos = gratuitos.filter(m => m !== modelo).slice(0, 2);
  return { modelo, respaldos };
}
...
/**
 * @param razonamiento  'bajo' pide al modelo que piense poco antes de
 *   responder. NO es un ahorro de dinero (con un gratuito todo cuesta USD 0):
 *   es lo que evita que la RESPUESTA SE CORTE.
 *
 *   🔴 MEDIDO EL 8-set-2026 con las bases reales. Estos modelos razonan en voz
 *   alta y ese pensamiento consume el mismo `max_tokens` que la respuesta. Con
 *   documentos largos el modelo gastaba TODO el techo pensando y la extracción
 *   moría con `finish_reason: 'length'`, que el usuario veía como «la respuesta
 *   se cortó por tamaño: analiza un rango más corto». Pasó en las cuatro
 *   pruebas de Gabriel: personal de las bases de ejecución (dos rangos), de las
 *   de supervisión, y el currículum entero.
 *
 *   Extraer datos citando literalmente NO necesita cadena de pensamiento: la
 *   respuesta está en el texto que se le dio. Bajar el razonamiento deja el
 *   techo para lo que importa, que es el JSON.
 */
export function construirCuerpo({ modelo, respaldos = [], politica = 'zdr', system, user, maxTokens, razonamiento = null }) {
  // 🔴 EL RECORTE NO ES COSMÉTICO: ES EL QUE EVITA UN APAGÓN TOTAL.
  // OpenRouter acepta COMO MÁXIMO 3 entradas en `models`. Con 4 no degrada ni
  // ignora las de más: rechaza la request ENTERA con
  //   400 "'models' array must have 3 items or fewer."
  // …y como el 400 pasa antes de mirar el modelo, se lleva puesto también al
  // titular, que andaba perfecto. El 15-set-2026 la cadena pasó a titular + 3
  // respaldos (= 4) y Captura Mágica dejó de leer TODA factura en producción:
  // cada lectura caía al respaldo de Claude, que estaba sin saldo, y la
  // almacenera veía "el servicio de IA no tiene crédito" — un mensaje que
  // señalaba el saldo cuando el problema era el largo de esta lista.
  // El slice es a propósito más fuerte que el default: OPENROUTER_STRUCT_FALLBACK
  // se configura en Vercel sin pasar por ningún test, así que una lista larga
  // ahí tiene que DEGRADAR (se usan los 3 primeros), nunca romper.
  const cadena = [modelo, ...respaldos].slice(0, MAX_MODELOS_CADENA);
  return {
    model: modelo,
    ...
    temperature: 0,
    // `exclude: true` pide además que el pensamiento NO vuelva en la respuesta.
    // Los modelos que no soportan el parámetro lo ignoran sin fallar.
    ...(razonamiento === 'bajo' ? { reasoning: { effort: 'low', exclude: true } } : {}),
    provider: {
...
export async function openrouterChat(apiKey, body, deadline, { intentos = 2 } = {}) {
  ...
  // 🔴 EL MENSAJE SE LOGUEA ACÁ Y NO EN EL LLAMADOR. Los callers venían
  // imprimiendo solo `e.upstreamStatus`, y cuando el 15-set-2026 OpenRouter
  // empezó a contestar 400 a TODAS las facturas, en Vercel se leía
  // "OpenRouter falló: 400" y nada más: hubo que reproducir la llamada a mano
  // contra la API para enterarse de que decía "'models' array must have 3
  // items or fewer". El motivo viene en el cuerpo — tirarlo era convertir un
  // diagnóstico de un minuto en uno de media hora.
  try {
    console.warn('[openrouter] falló definitivo:', JSON.stringify({
      codigo: ultimo?.codigo || 0,
      mensaje: (ultimo?.mensaje || '').slice(0, 300),
      proveedor: ultimo?.proveedor || null,
      modelos: [body?.model, ...(body?.models || []).slice(1)].filter(Boolean),
      politica: body?.provider?.zdr ? 'zdr' : (body?.provider?.data_collection || null),
    }));
  } catch {}
