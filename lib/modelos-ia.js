...
export const MODELOS_TEXTO = [
  ...
  // contra la API de OpenRouter el 15-set-2026: los modelos `:free` del
  ...
  //
  // 🔴 LA SEGUNDA MITAD TAMPOCO, Y ESTA LISTA LLEGÓ A MENTIRLO. Unas horas
  // antes, hoy mismo, acá entraron `nvidia/nemotron-3-super-120b-a12b:free` y
  // `google/gemma-4-31b-it:free` como elegibles «con ZDR y JSON forzado»,
  // leyendo el flag `zdr` del catálogo. Pedidos DE VERDAD con la key de
  // producción, los dos devuelven 404 "No endpoints found matching your data
  // policy". El flag del catálogo dice qué declara el modelo, no a qué
  // endpoints llega esta cuenta. Se los sacó: ofrecer en un desplegable un
  // modelo que siempre falla es peor que no ofrecerlo.
  //
  // Barridos los 20 gratuitos uno por uno, pasan ZDR TRES, todos de la misma
  // familia y el mismo proveedor (Novita): ling-3.0-flash en sus variantes
  // -fin (titular de la cadena), -vl y -sante. El único que se agrega acá es
  // -vl, que es además el respaldo real de 'auto'.
  {
    id: 'inclusionai/ling-3.0-flash-vl:free',
    nombre: 'Ling 3.0 Flash VL (gratis)',
    gratis: true,
    precio: { entrada: 0, salida: 0 },
    detalle: 'Hermano del titular de la cadena gratis (misma familia, mismo proveedor: Novita), con 262 k de contexto. Es uno de los TRES gratuitos del catálogo que hoy pasan de verdad el filtro de Zero Data Retention —verificado con una llamada real, no con el flag del catálogo— y el respaldo al que cae \'auto\' cuando el titular se satura.',
    ojo: 'Un solo proveedor, y el MISMO que sirve al titular: si Novita se cae, se caen los dos juntos. No es una segunda opinión independiente. Comparte también el flojo de la familia: no admite JSON forzado y razona en voz alta antes de contestar.',
  },
  {
