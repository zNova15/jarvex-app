...
export const MODELOS_TEXTO = [
  ...
  // ── LOS GRATUITOS QUE SE PUEDEN ELEGIR A PROPÓSITO (tanda 9, 15-set-2026) ──
  //
  // Pedido de Gabriel: «estaba pensando en utilizar una IA gratuita de
  // OpenRouter que tenga muchos servidores para que analicen bien sin
  // necesidad de usar una IA de pago, y eso para las tres secciones».
  //
  // 🔴 LA PRIMERA MITAD DEL PEDIDO NO EXISTE, Y CONVIENE SABERLO. Medido
  // contra la API de OpenRouter el 15-set-2026: los 23 modelos `:free` del
  // catálogo tienen UN SOLO endpoint cada uno. `openai/gpt-oss-120b` tiene
  // VEINTICUATRO. Esa multiplicidad de proveedores ES lo que se compra al
  // pagar: un `:free` es un proveedor regalando capacidad, y por eso se satura
  // y por eso la lista rota cada pocos meses. No hay un gratuito con muchos
  // servidores; lo más parecido es una CADENA de gratuitos, que es lo que ya
  // hace 'auto' (ver MODELO_FALLBACK_DEFAULT en lib/openrouter.js).
  //
  // LA SEGUNDA MITAD SÍ MEJORÓ, Y MUCHO. Cuando se escribió openrouter.js
  // quedaba UN gratuito que pasara el filtro `zdr=true`; hoy hay DOCE. Y dos de
  // ellos aceptan JSON forzado (`response_format` / `structured_outputs`), que
  // es justo la debilidad que tiene anotada la cadena gratis de hoy («no admite
  // JSON forzado»): con eso desaparece el «la IA no devolvió una respuesta
  // usable», que es el error que hace repetir preguntas ya pagadas.
  //
  // Se agregan como ELEGIBLES, no como default: cambiar el titular sin medir
  // sería cambiar un 94% conocido por un número que nadie corrió. El piloto es
  // `scripts/piloto-clasificacion.mjs` y corre contra los mismos 32 casos.
  {
    id: 'nvidia/nemotron-3-super-120b-a12b:free',
    nombre: 'Nemotron 3 Super 120B (gratis)',
    gratis: true,
    precio: { entrada: 0, salida: 0 },
    detalle: 'El gratuito más serio del catálogo hoy: 120B con 12B activos (MoE), 262 k de contexto, ZDR y —lo importante acá— JSON FORZADO, que la cadena gratis de siempre no tiene. Es el análogo gratuito de GPT-OSS 120B. Servido por NVIDIA.',
    ojo: 'UN SOLO proveedor (NVIDIA), como todos los `:free`. Eso significa que cuando se satura no hay a dónde caerse dentro del mismo modelo: si lo elegís a mano no lleva cadena de respaldo, y si falla, falla. Para un barrido de 875 preguntas seguidas eso se nota.',
    recomendadoEn: [],
  },
  {
    id: 'google/gemma-4-31b-it:free',
    nombre: 'Gemma 4 31B (gratis)',
    gratis: true,
    precio: { entrada: 0, salida: 0 },
    detalle: 'Chico, rápido, con ZDR y JSON forzado. 262 k de contexto. Sirve como segunda opinión gratuita y como respaldo del Nemotron. Servido por Google AI Studio.',
    ojo: 'Un solo proveedor, y 31B es bastante menos modelo que los de 120B: en las descripciones difíciles —una marca comercial sin contexto— se le nota. Sin medir contra el piloto, no hay motivo para preferirlo a la cadena de siempre.',
  },
  {
    id: 'openai/gpt-oss-120b',
    nombre: 'GPT-OSS 120B',
    precio: { entrada: 0.037, salida: 0.17 },
    detalle: 'El de pesos abiertos de OpenAI, y el más barato del catálogo con ZDR: 4 veces menos que GLM 5.3 Flash a la entrada y 3 a la salida. 24 proveedores (Groq, Cerebras, Together, Bedrock…) —verificado otra vez el 15-set, y es exactamente lo que ningún gratuito tiene—, así que rara vez está saturado. SEGUNDO piloto, 15-set por la noche, ya con las reglas de desempate de la tanda 3 en el prompt: 30 de 32 (94%), 0 respuestas fuera de lista, 0 llamadas fallidas, las gemelas del mismo tubo en el mismo código, y un barrido completo de una empresa (875 descripciones) por USD 0,11 — la mitad que Luna. El único error que le queda es REJILLAS METÁLICAS, el caso que los tres modelos discuten.',
    recomendadoEn: ['clasificacion'],
  },
  {
    id: 'z-ai/glm-5.3-flash',
    nombre: 'GLM 5.3 Flash',
    precio: { entrada: 0.07, salida: 0.233 },
    detalle: 'El más barato de los pagos con contexto grande (1,3 M). 27 proveedores, así que rara vez está saturado.',
    ojo: 'Z.ai es china (sede en Singapur); con la política ZDR el proveedor no guarda el pedido. En los DOS pilotos del 15-set quedó último y empeoró: 28 de 32 (88%), 3 respuestas fuera de lista y 3 sin JSON válido — 6 preguntas pagadas que el server tuvo que descartar. Y es el más caro por barrido de los tres (USD 0,29 contra 0,11).',
  },
  {
    id: 'openai/gpt-5.6-luna',
    nombre: 'GPT-5.6 Luna',
    precio: { entrada: 0.20, salida: 1.20 },
    detalle: 'Contexto de 1,05 M, JSON forzado y esfuerzo de razonamiento regulable. Servido desde EE. UU. (OpenAI/Azure/Bedrock). En el PRIMER piloto (15-set, mañana) ganó con 97% y fue el único que no repitió el bug de Faber Castell — por eso estuvo recomendado.',
    ojo: 'DEJÓ DE SER EL RECOMENDADO en el segundo piloto (15-set, noche). Dos motivos, y el segundo pesa más: (1) la razón por la que ganaba desapareció — las reglas de desempate de la tanda 3 arreglaron Faber Castell para TODOS los modelos, así que ya no lo distingue nada; (2) 6 de 32 llamadas fallaron con 429 «rate-limited upstream», o sea que su 92% está medido sobre 26 casos y no sobre 32. Un modelo que no contesta una de cada cinco veces no sirve para un barrido de 875 descripciones seguidas, por bien que conteste cuando contesta. Cuesta además el doble que GPT-OSS 120B.',
  },
  {
...
export const DEFAULTS = {
  ...
  // TERCER ÁMBITO (tanda 2, 15-set-2026): clasificar insumos y servicios contra
  // el IUPC. No lee documentos —no hay OCR que elegir acá, la clave ni se
  // guarda— pero sí es el que más LLAMADAS hace: un barrido completo son ~875
  // preguntas seguidas, contra una por factura en Captura Mágica.
  //
  // Por qué merece ámbito propio y no heredar el de Captura: son dos tareas
  // distintas. Captura le pide al modelo que TRANSCRIBA un documento largo;
  // acá se le pide que ELIJA una opción de una lista corta con la evidencia de
  // la norma delante. El mejor modelo para una no es el mejor para la otra, y
  // el volumen de llamadas hace que el precio pese distinto.
  //
  // Arranca en 'auto' (la cadena de gratuitos, lo que ya venía haciendo): sin
  // esto la tanda cambiaría el motor sin que nadie lo decida. El titular
  // definitivo lo fija Gabriel con el piloto — `scripts/piloto-clasificacion.mjs`.
  clasificacion: { ocr: 'mistral-ocr-2512', texto: 'auto' },
};
