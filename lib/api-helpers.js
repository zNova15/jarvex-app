...
// `export default` (la cuenta es Pro y ya no tiene el tope de 12 de Hobby,
// pero cada function de más sigue siendo superficie de más para autenticar).
...
//
// Cache en memoria del lambda, por token, para no repetir /auth/v1/user +
// profiles en llamadas consecutivas del mismo request "en ráfaga" (ej. r2
// firmando 20 fotos de una galería en unos segundos). TTL corto a propósito:
// una cuenta recién desactivada no debe seguir pasando por más de este rato.
...
export async function requireAuth(req) {
  ...
  if (cached && Date.now() < cached.expiresAt) {
    if (cached.profile?.activo === false) throw httpError(403, 'Cuenta desactivada');
    return { user_id: cached.user_id, profile: cached.profile, token };
  }
  ...
  _authCache.set(token, { user_id: userId, profile, expiresAt: Date.now() + AUTH_CACHE_TTL_MS });
...
// Corta con 403 si el rol del perfil no está en la allowlist del endpoint.
// Uso: const ctx = await requireAuth(req); requireRole(ctx, ['admin','contador']);
export function requireRole(ctx, roles) {
  if (!roles.includes(ctx?.profile?.rol)) {
    throw httpError(403, 'Tu rol no puede usar este servicio');
  }
}

// Inverso de requireRole: para endpoints que usan CASI todos los roles
// operativos y solo necesitan sacar un par (ej. la cuenta compartida 'campo'
// o 'solo_lectura'), listar la excepción es más corto y más difícil de
// desactualizar que una allowlist de 15 roles.
export function requireRoleExcept(ctx, rolesExcluidos) {
  if (rolesExcluidos.includes(ctx?.profile?.rol)) {
    throw httpError(403, 'Tu rol no puede usar este servicio');
  }
}
...
// Para las respuestas que arman su propio JSON de error (no pasan por
// sanitizeError) pero igual quieren mandar `detail` solo fuera de producción.
// Uso: res.status(500).json({ error: '...', ...detalleDev(e.message) })
export function detalleDev(value) {
  if (process.env.NODE_ENV === 'production') return {};
  return { detail: String(value ?? '').slice(0, 400) };
}
